...
#
# simulating data for landscape degradation
# 
...
## Data simulation:
source(paste0(fundir, "sim_obsDat_landdeg_function_faster_100425.R"))
...
ntime <- 300 # number of total years, 300 for degradation only scenario
...
ntime_stop <- 100 # 100 years of no interference after stopping everything
...
# save metapopulation capacity over time too
...
for(land in 1:nlands){ # 10 landscape repetitions
  
  print(paste("Running landscape repetition", land, ":"))
  
  # calculate new lands
  lands_new <- patchID_new <- vector(mode = "list", length = (1 + ntime_deg))
  dmat_new <- S_new <- vector(mode = "list", length = (1 + ntime_deg))
  
  # naming based on years
  names(lands_new) <- (ntime_first):(ntime_first + ntime_deg)
  names(dmat_new) <- (ntime_first):(ntime_first + ntime_deg)
  names(S_new) <- (ntime_first):(ntime_first + ntime_deg)
  names(patchID_new) <- (ntime_first):(ntime_first + ntime_deg)
  ...
  # after_years # after stopping degradation and restoration
  ...
  dimensions <- vector(mode = "list", length = 1 + ntime_deg)
  ...
  names(M_all) <- (ntime_first):(ntime_first + ntime_deg)
  ...
  for(t in (ntime_first+1):(ntime_first+ntime_deg)){
    lambdaM[t] <- Re(eigen(M_all[[as.character(t)]])$values[1])
  }
  
  lambdaM[max(deg_years):ntime] <- lambdaM[max(deg_years)]
  ...
  for(e in e.values){
    ...
    for(c in c.values){
      ...
      out <- sim_obsDat_landdeg_100425(nsim = nsim, npatch = npatch, ntime = ntime,
  ...
  rm(sim_land, initial_land, initial_dmat, initial_S,  dmat_new, S_new, 
     patchID_new, lands_new, to_remove, which_patch_removed_per_year, lambdaM, lambdaM_df);gc()
} # end nlandscapes
