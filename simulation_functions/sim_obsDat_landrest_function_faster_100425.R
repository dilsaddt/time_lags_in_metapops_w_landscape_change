...
# ### land restoration parts
# ### #####################################################
#
# Upgrade to 07/01/2025 versio: Made the code faster
#
# Date:10/04/2025
...
# ntime = total nummber of time (years) spanning entire simulation - should be equal to sum of ntime_first, ntime_deg, ntime_rest
# 
# ntime_first = length of equilibrium years
# ntime_deg = length of degradation years
# ntime_rest = length of rest years
#
...
sim_obsDat_landrest_100425 <- function(nsim, npatch, ntime,
                                       ntime_first, ntime_deg, ntime_rest, ntime_stop,
  ...
  rest_npatch <- nrow(lands_new[[as.character(ntime_first + ntime_deg)]])
  rest_dmat <- dmat_new[[as.character(ntime_first + ntime_deg)]]
  rest_S <- c(S_new[[as.character(ntime_first + ntime_deg)]])
  ...
  for(d in 1:nsim){
    ...
    for(t in 2:ntime_first){
      for(i in 1:init_npatch){
        ...
        C[i,t-1] <- c * (z[1:init_npatch,t-1] %*% con[i,1:init_npatch,t-1])
    ...
    for (t in rest_years) {
      S_vec <- S_new[[as.character(t)]][,1]
      n <- length(S_vec)
      
      # Extinction rate and probability
      E[1:n, t-1] <- e / (S_vec^x)
      eps[1:n, t-1] <- 1 - exp(-E[1:n, t-1])
    ...
    # colonization probability and true state
    
    for(t in rest_years){
      
      # first for still existing patches from previous time step
      for(i in which(!is.na(z[,t-1]))){ 
        
        C[i, t-1] <- c * (z[which(!is.na(z[,t-1])), t-1] %*% con[i, which(!is.na(z[,t-1])), t-1])
      ...
      ## this is if we assume the true state (z) of newly restored patches are 0 at the time they are restored
      # then the newly added patches
      add <- nrow(lands_new[[as.character(t)]]) - length(which(!is.na(z[,t-1])))
      
      for(i in (max(which(!is.na(z[,t-1]))) + 1):(max(which(!is.na(z[,t-1]))) + add)){ 
        
        # we assume the newly patches will not be colonized at the time they are restored
        z[i, t] <- 0
      }
    }
    
    ####################################
    ##### AFTER DEG AND REST YEARS #####
    ####################################
...
# out <- sim_obsDat_landrest_070125(nsim = nsim, npatch = npatch, ntime = ntime,
